...
class MixMicrobleed(SegmentationAlgorithm):
    def __init__(self):
    ...
    def predict(self, *, input_images: List[SimpleITK.Image]) -> List[SimpleITK.Image]:
        ...
        for i in range(len(self.input_modalities)):
            SimpleITK.WriteImage(input_images[i], '/home/'+self.input_modalities[i]+'.nii.gz')
        
        subprocess.run(["python", "/home/MCBS_predictions.py"])

        print("==> Prediction done")

        return [SimpleITK.ReadImage('/home/predictions.nii.gz')]


if __name__ == "__main__":
    MixMicrobleed().process()
